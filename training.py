...
# TODO:
# - Document better the experiment class, maybe with some default configuration
# - Create models on demand, not in bulk, in ExperimentsSet


class NBatchLogger(Callback):
    """
    A TF Logger that prints last performance per `display` epochs.
    """
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if epoch % self.display == 0:
            ...
            for k in ['loss', 'val_loss']:
                val = self.metric_cache[k]
                if abs(val) > 1e-3:
                    metrics_log += ' - %s: %.4f' % (k, val)
                else:
                    metrics_log += ' - %s: %.4e' % (k, val)
            print('Epoch: {}/{} ... {} - overfit: {}'.format(epoch,
                                                             self.params['epochs'],
                                                             metrics_log,
                                                             metrics_log['val_loss'] / metrics_log['loss']))
            self.metric_cache.clear()


class ExperimentLogger(Callback):
    """
    A TF Logger that log average performance per `display` steps.
    """
...
class Experiment:
    ...
    def run(self, display=-1, save=False):
        ...
        last_ts = 0
        if len(self.el.exp_log) > 0:
            last_ts = self.el.exp_log[-1]['timestamp']
        self.el.start_ts = time.time() - last_ts
        self.model.fit(x=self.inputs, y=self.outputs, **kwargs)
        if save:
            self.write()

    def resume(self, epochs=200, display=-1, save=False):
        kwargs = self.fit_params
        kwargs["callbacks"] = [self.el]
